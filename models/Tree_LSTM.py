...
class ChildSumTreeLSTM(nn.Module):
    ...
    def node_forward(self, x, child_c, child_h):
        ...
        u = torch.sigmoid(self.ux(x) + self.uh(child_h_sum))
        # print('i.size():', i.size())
...
class BatchChildSumTreeLSTM(nn.Module):
    ...
    def node_forward(self, x, child_c, child_h):
        ...
        fx = torch.unsqueeze(self.fx(x), 1)
        if self.debug:
            print('fx.size():', fx.size())  # torch.Size([4, 1, 100])
        # child_h: (4, 1, 100)
        fx = fx.view(fx.size(0), 1, fx.size(2)).expand(fx.size(0), child_h.size(1), fx.size(2))
        f = self.fh(child_h) + fx
        if self.debug:
            print('f.size():', f.size())
        f = torch.sigmoid(f)
        fc = torch.squeeze(F.torch.mul(f, child_c), 1)
    ...
    def forward(self, x, bfs_tensor, children_batch_list):
        ...
        for index in range(sent_len):
            ...
            rel_max, _ = torch.max(rels, 1)
            if self.debug:
                print('rel_max:', rel_max)
                print('rel_max.size():', rel_max.size())  # torch.Size([4])
            rel_batch_max = torch.max(rel_max, 0)[0]
            c, h = None, None
            if rel_batch_max.data.tolist() == 0:
                ...
                for i, batch in enumerate(pad_c):
                    for j in range(rel_max.data.tolist()[i]):
