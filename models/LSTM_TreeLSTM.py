# Version python3.6
# -*- coding: utf-8 -*-
# @Time    : 2018/11/17 10:23 PM
# @Author  : zenRRan
# @Email   : zenrran@qq.com
# @File    : LSTM_TreeLSTM.py
...
class LSTM_TreeLSTM(nn.Module):
    def __init__(self, opts, vocab, label_vocab):
        ...
        self.linear1 = nn.Linear(self.hidden_size * (self.flag + 1), self.hidden_size // 2)
    ...
    def forward(self, x, bfs_tensor, children_batch_list):
        ...
        #lstm
        lstm_out, _ = self.lstm(x)  # [1, 1, 200]
        lstm_out = torch.transpose(lstm_out, 1, 2)
        lstm_out = torch.tanh(lstm_out)
        lstm_out = F.max_pool1d(lstm_out, lstm_out.size(2))  # [1, 200, 1]
        lstm_out = lstm_out.squeeze(2)  # [1, 400]
        ...
        out = torch.cat((lstm_out, treelstm_out), 1)
